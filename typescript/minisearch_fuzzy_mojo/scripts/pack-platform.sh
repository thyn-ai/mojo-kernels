...
# compiled msmojo kernel, then repair it: vendor the Mojo runtime shared
...
case "$(uname -s)-$(uname -m)" in
  Darwin-arm64)  pkg_dir="packages/darwin-arm64"; lib="libmsmojo.dylib" ;;
  Linux-x86_64)  pkg_dir="packages/linux-x64";  lib="libmsmojo.so" ;;
  *) echo "error: no platform package for $(uname -s)-$(uname -m)" >&2; exit 1 ;;
esac

src="${MINISEARCH_MOJO_NATIVE_SRC:-$repo_root/kernels/minisearch_fuzzy/build/$lib}"
