...
# compiled ckmeansmojo kernel, then repair it: vendor the Mojo runtime shared
...
# `pixi run bash typescript/ckmeans-mojo/scripts/pack-platform.sh`.
#
...
case "$(uname -s)-$(uname -m)" in
  Darwin-arm64)  pkg_dir="packages/darwin-arm64"; lib="libckmeansmojo.dylib" ;;
  Linux-x86_64)  pkg_dir="packages/linux-x64";  lib="libckmeansmojo.so" ;;
  *) echo "error: no platform package for $(uname -s)-$(uname -m)" >&2; exit 1 ;;
esac

src="${CKMEANS_MOJO_NATIVE_SRC:-$repo_root/kernels/ckmeans/build/$lib}"
