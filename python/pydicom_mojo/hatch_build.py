"""Hatch build hook: bundle the compiled rlemojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/pydicom-rle/build.sh` (run it before building the wheel) and is
tagged `py3-none-<platform>`.

Escape hatch: `PYDICOM_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "librlemojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "pydicom-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("PYDICOM_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("PYDICOM_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "pydicom-rle", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/pydicom-rle/build.sh` from the repository root), or set "
                "PYDICOM_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"pydicom_mojo/_native/{os.path.basename(src)}"
