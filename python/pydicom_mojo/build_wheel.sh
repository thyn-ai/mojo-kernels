#!/usr/bin/env bash
# Build the pydicom-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's librlemojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/pydicom_mojo/build_wheel.sh`).
#
