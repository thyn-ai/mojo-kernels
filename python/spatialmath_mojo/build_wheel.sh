#!/usr/bin/env bash
# Build the spatialmath-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's libspatialmathmojo resolves its runtime
# dependencies through an absolute rpath into the build machine's pixi
# environment, so it only ever loads on the machine that built it —
...
# (`pixi run bash python/spatialmath_mojo/build_wheel.sh`).
#
