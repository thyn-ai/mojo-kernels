"""Hatch build hook: bundle the compiled spatialmathmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/spatialmath/build.sh` (run it, e.g. `pixi run bash
kernels/spatialmath/build.sh`, before building the wheel) and is tagged
`py3-none-<platform>`.

Escape hatch: `SPATIALMATH_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libspatialmathmojo.dylib",
    "linux": "libspatialmathmojo.so",
    "win32": "spatialmathmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libspatialmathmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "spatialmath-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("SPATIALMATH_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("SPATIALMATH_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(
                self.root, "..", "..", "kernels", "spatialmath", "build", _lib_name()
            )
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/spatialmath/build.sh` inside the repo pixi "
                "environment), or set SPATIALMATH_MOJO_ALLOW_PURE_WHEEL=1 to "
                "build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"spatialmath_mojo/_native/{os.path.basename(src)}"
