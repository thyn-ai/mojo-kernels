#!/usr/bin/env bash
# Build the jmespath-mojo platform wheel (wheel-only; no sdist), then repair
# it: vendor the Mojo runtime shared libraries into the wheel and rewrite the
# kernel library's load paths to be wheel-relative ($ORIGIN / @loader_path).
# Without this step the wheel's libjmespathmojo resolves its runtime
# dependencies through an absolute rpath into the build machine's pixi
# environment, so it only ever loads on the machine that built it — everywhere
# else the wrapper silently falls back. dist/ contains the repaired,
# self-contained wheel.
#
# Run inside the repo pixi environment (`pixi run bash
# python/jmespath_mojo/build_wheel.sh`).
#
