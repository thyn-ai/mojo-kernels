#!/usr/bin/env bash
# Build the difflib-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libdifflibmojo resolves its runtime
...
#
# Run inside the repo pixi environment (`pixi run bash python/difflib_mojo/build_wheel.sh`).
#
