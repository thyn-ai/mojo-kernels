...
- Wheels are **per-platform** (`py3-none-macosx_*_arm64`,
  ...
  **self-contained**: `delocate` (macOS) / `auditwheel repair` (Linux)
  ...
  `PYKALMAN_MOJO_ALLOW_PURE_WHEEL=1` (the wheel CI builds and tests on Windows: [`windows-fallback`](https://github.com/thyn-ai/mojo-kernels/actions/workflows/windows-fallback.yml)).
