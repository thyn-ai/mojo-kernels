"""Hatch build hook: bundle the compiled mistunemojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/mistune/build.sh` (run it, or `pixi run bash kernels/mistune/build.sh`,
before building the wheel) and is tagged `py3-none-<platform>`.

Escape hatch: `MISTUNE_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
vendored fallback engine on every platform. This exists for metadata smoke
tests and for platforms with no Mojo toolchain (e.g. a Windows wheel), never
for the primary macOS/Linux release wheels.
"""
...
_LIB_NAMES = {
    "darwin": "libmistunemojo.dylib",
    "linux": "libmistunemojo.so",
    "win32": "mistunemojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libmistunemojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "mistune-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("MISTUNE_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("MISTUNE_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "mistune", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`pixi run bash kernels/mistune/build.sh` from the repository root), or set "
                "MISTUNE_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"mistune_mojo/_native/{os.path.basename(src)}"
