#!/usr/bin/env bash
# Build the mistune-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libmistunemojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/mistune_mojo/build_wheel.sh`).
#
