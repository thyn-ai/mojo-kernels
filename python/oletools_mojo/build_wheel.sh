#!/usr/bin/env bash
# Build the oletools-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's libvbamojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/oletools_mojo/build_wheel.sh`).
#
