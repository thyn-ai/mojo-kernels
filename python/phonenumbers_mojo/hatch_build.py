"""Hatch build hook: bundle the compiled phonenumbersmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/phonenumbers/build.sh` (run it before building the wheel) and is
tagged `py3-none-<platform>`.

Escape hatch: `PHONENUMBERS_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
fallback engine on every platform. This exists for metadata smoke tests and
...
_LIB_NAMES = {
    "darwin": "libphonenumbersmojo.dylib",
    "linux": "libphonenumbersmojo.so",
    "win32": "phonenumbersmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libphonenumbersmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "phonenumbers-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("PHONENUMBERS_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("PHONENUMBERS_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "phonenumbers", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/phonenumbers/build.sh` from the repository root), or set "
                "PHONENUMBERS_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"phonenumbers_mojo/_native/{os.path.basename(src)}"
