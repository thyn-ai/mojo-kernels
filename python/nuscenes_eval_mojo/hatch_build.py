"""Hatch build hook: bundle the compiled nuscenesevalmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/nuscenes-eval/build.sh` (run it first) and is tagged
`py3-none-<platform>`.

Escape hatch: `NUSCENES_EVAL_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libnuscenesevalmojo.dylib",
    "linux": "libnuscenesevalmojo.so",
    "win32": "nuscenesevalmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libnuscenesevalmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "nuscenes-eval-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("NUSCENES_EVAL_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("NUSCENES_EVAL_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "nuscenes-eval", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`pixi run bash kernels/nuscenes-eval/build.sh` from the repository root), or set "
                "NUSCENES_EVAL_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"nuscenes_eval_mojo/_native/{os.path.basename(src)}"
