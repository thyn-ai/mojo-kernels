#!/usr/bin/env bash
# Build the pefile-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libpefilemojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (or with .pixi/envs/default/bin on PATH).
#
