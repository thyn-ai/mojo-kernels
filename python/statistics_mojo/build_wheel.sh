#!/usr/bin/env bash
# Build the statistics-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's libstatsmojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/statistics_mojo/build_wheel.sh`).
#
