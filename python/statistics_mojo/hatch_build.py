"""Hatch build hook: bundle the compiled statsmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/statistics/build.sh` (run it before building the wheel) and is
tagged `py3-none-<platform>`.

Escape hatch: `STATISTICS_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
stdlib-delegating fallback on every platform. This exists for metadata smoke
...
_LIB_NAMES = {
    "darwin": "libstatsmojo.dylib",
    "linux": "libstatsmojo.so",
    "win32": "statsmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libstatsmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "statistics-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("STATISTICS_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("STATISTICS_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "statistics", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`pixi run bash kernels/statistics/build.sh` from the repository root), or set "
                "STATISTICS_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"statistics_mojo/_native/{os.path.basename(src)}"
