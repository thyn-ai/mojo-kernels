#!/usr/bin/env bash
# Build the cclib-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libgaussgridmojo resolves its runtime
...
#
# Run inside the repo pixi environment (`pixi run wheel-cclib`).
#
