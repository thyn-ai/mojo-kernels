#!/usr/bin/env bash
# Build the vader-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libvadermojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment, from the repository root:
#   pixi run bash python/vader_mojo/build_wheel.sh
#
