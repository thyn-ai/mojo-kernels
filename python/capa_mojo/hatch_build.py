"""Hatch build hook: bundle the compiled capamojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/capa/build.sh` (run it first) and is tagged
`py3-none-<platform>`.

Escape hatch: `CAPA_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
pure-Python fallback on every platform. This exists for metadata smoke tests
...
_LIB_NAMES = {
    "darwin": "libcapamojo.dylib",
    "linux": "libcapamojo.so",
    "win32": "capamojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libcapamojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "capa-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("CAPA_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("CAPA_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "capa", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/capa/build.sh` from the repository root), or set "
                "CAPA_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"capa_mojo/_native/{os.path.basename(src)}"
