#!/usr/bin/env bash
# Build the capa-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libcapamojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/capa_mojo/build_wheel.sh`).
#
