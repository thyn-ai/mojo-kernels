#!/usr/bin/env bash
# Build the sacrebleu-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's libsacremojo resolves its runtime dependencies
...
#
# Run from the repository root after `bash kernels/sacrebleu/build.sh`:
#   bash python/sacrebleu_mojo/build_wheel.sh
#
