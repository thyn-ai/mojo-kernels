"""Hatch build hook: bundle the compiled bm25mojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/bm25/build.sh` (run it, or `pixi run build-kernel-bm25`, before
building the wheel) and is tagged `py3-none-<platform>`.

Escape hatch: `BM25_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libbm25mojo.dylib",
    "linux": "libbm25mojo.so",
    "win32": "bm25mojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libbm25mojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "bm25-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("BM25_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("BM25_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "bm25", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`pixi run build-kernel-bm25` from the repository root), or set "
                "BM25_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"bm25_mojo/_native/{os.path.basename(src)}"
