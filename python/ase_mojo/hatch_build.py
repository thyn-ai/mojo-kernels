"""Hatch build hook: bundle the compiled aseneighborlistmojo kernel into a wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/ase_neighborlist/build.sh` (run it before building the wheel) and
is tagged `py3-none-<platform>`.

Escape hatch: `ASE_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libaseneighborlistmojo.dylib",
    "linux": "libaseneighborlistmojo.so",
    "win32": "aseneighborlistmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libaseneighborlistmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "ase-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("ASE_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("ASE_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(
                self.root,
                "..",
                "..",
        ...
        if not os.path.isfile(src):
            raise FileNotFoundError(
                ...
                "(`bash kernels/ase_neighborlist/build.sh` from the repository root), or set "
                "ASE_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"ase_mojo/_native/{os.path.basename(src)}"
