...
- Wheels are **per-platform** (`py3-none-macosx_*_arm64`,
  ...
  **self-contained**: `delocate` (macOS) / `auditwheel repair` (Linux)
  ...
  `ASE_MOJO_ALLOW_PURE_WHEEL=1` (the wheel CI builds and tests on Windows: [`windows-fallback`](https://github.com/thyn-ai/mojo-kernels/actions/workflows/windows-fallback.yml)).
...
- macOS arm64 and Linux x86_64 native; everything else (including Windows — tested there in CI: [`windows-fallback`](https://github.com/thyn-ai/mojo-kernels/actions/workflows/windows-fallback.yml)) runs the NumPy fallback — silently, bit-identically, ~10×
