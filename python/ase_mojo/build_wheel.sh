#!/usr/bin/env bash
# Build the ase-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libaseneighborlistmojo resolves its runtime
...
#
# Run with the repo toolchain on PATH, e.g. from the repository root:
#   pixi run bash python/ase_mojo/build_wheel.sh
#
