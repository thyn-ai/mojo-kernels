"""Hatch build hook: bundle the compiled ipaddressmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/ipaddress/build.sh` (run it before building the wheel) and is
tagged `py3-none-<platform>`.

Escape hatch: `IPADDRESS_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
fallback on every platform. This exists for metadata smoke tests and for
platforms with no Mojo toolchain (e.g. a Windows wheel), never for the
...
_LIB_NAMES = {
    "darwin": "libipaddressmojo.dylib",
    "linux": "libipaddressmojo.so",
    "win32": "ipaddressmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libipaddressmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "ipaddress-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("IPADDRESS_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("IPADDRESS_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "ipaddress", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/ipaddress/build.sh` with the repo's pixi toolchain "
                "on PATH), or set IPADDRESS_MOJO_ALLOW_PURE_WHEEL=1 to build a "
                "fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"ipaddress_mojo/_native/{os.path.basename(src)}"
