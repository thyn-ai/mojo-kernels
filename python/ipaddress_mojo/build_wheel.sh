#!/usr/bin/env bash
# Build the ipaddress-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libipaddressmojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (python + the build tooling on PATH).
#
