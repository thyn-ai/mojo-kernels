#!/usr/bin/env bash
# Build the vol-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libvolatility3hivemojo resolves its runtime
...
#
# Run inside the repo pixi environment (`pixi run bash python/vol_mojo/build_wheel.sh`).
#
