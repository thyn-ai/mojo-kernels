"""Hatch build hook: bundle the compiled uprootmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/uproot-branches/build.sh` (run it before building the wheel) and
is tagged `py3-none-<platform>`.

Escape hatch: `UPROOT_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libuprootmojo.dylib",
    "linux": "libuprootmojo.so",
    "win32": "uprootmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libuprootmojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "uproot-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("UPROOT_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("UPROOT_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(
                self.root,
                "..",
                "..",
        ...
        if not os.path.isfile(src):
            raise FileNotFoundError(
                ...
                "(`bash kernels/uproot-branches/build.sh` from the repository root), or set "
                "UPROOT_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"uproot_mojo/_native/{os.path.basename(src)}"
