#!/usr/bin/env bash
# Build the obspy-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libkonnomojo resolves its runtime dependencies
...
#   ~/.pixi/bin/pixi run bash python/obspy_mojo/build_wheel.sh
#
