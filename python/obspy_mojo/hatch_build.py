"""Hatch build hook: bundle the compiled konnomojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/obspy-konno/build.sh` (run it before building the wheel) and is
tagged `py3-none-<platform>`.

Escape hatch: `OBSPY_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
...
_LIB_NAMES = {
    "darwin": "libkonnomojo.dylib",
    "linux": "libkonnomojo.so",
    "win32": "konnomojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libkonnomojo.so")
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "obspy-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("OBSPY_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("OBSPY_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "obspy-konno", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/obspy-konno/build.sh` inside the repo pixi "
                "environment), or set OBSPY_MOJO_ALLOW_PURE_WHEEL=1 to build a "
                "fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"obspy_mojo/_native/{os.path.basename(src)}"
