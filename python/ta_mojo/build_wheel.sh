#!/usr/bin/env bash
# Build the ta-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libtamojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/ta_mojo/build_wheel.sh`).
#
