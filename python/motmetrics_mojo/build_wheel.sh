#!/usr/bin/env bash
# Build the motmetrics-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's libmotmojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/motmetrics_mojo/build_wheel.sh`).
#
