"""Hatch build hook: bundle the compiled tomlmojo kernel into a platform wheel.

The wheel for the current platform force-includes the shared library compiled
by `kernels/toml/build.sh` (run it before building the wheel) and is tagged
`py3-none-<platform>`.

Escape hatch: `TOML_MOJO_ALLOW_PURE_WHEEL=1` builds a pure-Python
`py3-none-any` wheel with no native library — the resulting package uses the
stdlib-tomllib fallback on every platform. This exists for metadata smoke
...
_LIB_NAMES = {
    "darwin": "libtomlmojo.dylib",
    "linux": "libtomlmojo.so",
    "win32": "tomlmojo.dll",
}


def _lib_name() -> str:
    return _LIB_NAMES.get(sys.platform, "libtomlmojo.so")
...
def _platform_wheel_tag() -> str:
    ...
    falls back to stdlib tomllib, so these tags only ever widen compatibility.
    """
...
class CustomBuildHook(BuildHookInterface):
    PLUGIN_NAME = "toml-mojo-native"

    def initialize(self, version: str, build_data: dict) -> None:  # noqa: ARG002
        if os.environ.get("TOML_MOJO_ALLOW_PURE_WHEEL") == "1":
            build_data["pure_python"] = True
            build_data["infer_tag"] = False
            build_data["tag"] = "py3-none-any"
            return

        src = os.environ.get("TOML_MOJO_NATIVE_SRC") or os.path.abspath(
            os.path.join(self.root, "..", "..", "kernels", "toml", "build", _lib_name())
        )
        if not os.path.isfile(src):
            raise FileNotFoundError(
                f"native kernel not found at {src}. Compile it first "
                "(`bash kernels/toml/build.sh` inside the repo pixi environment), or set "
                "TOML_MOJO_ALLOW_PURE_WHEEL=1 to build a fallback-only wheel."
            )

        force_include = build_data.setdefault("force_include", {})
        force_include[src] = f"toml_mojo/_native/{os.path.basename(src)}"
