#!/usr/bin/env bash
# Build the slugify-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libslugifymojo resolves its runtime dependencies
...
#
# Run inside the repo pixi environment (`pixi run bash python/slugify_mojo/build_wheel.sh`).
#
