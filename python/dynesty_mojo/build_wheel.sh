#!/usr/bin/env bash
# Build the dynesty-mojo platform wheel (wheel-only; no sdist), then repair it:
...
# Without this step the wheel's libdynestymojo resolves its runtime dependencies
...
# (`pixi run bash python/dynesty_mojo/build_wheel.sh`).
#
