#!/usr/bin/env bash
# Build the langdetect-mojo platform wheel (wheel-only; no sdist), then repair
...
# Without this step the wheel's liblangdetectmojo resolves its runtime
...
#   ~/.pixi/bin/pixi run bash python/langdetect_mojo/build_wheel.sh
#
