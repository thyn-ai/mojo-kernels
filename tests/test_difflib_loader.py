"""Loader/backend behaviour for difflib-mojo: ABI handshake, fallback
forcing, diagnostics, and native/engine routing."""

from __future__ import annotations

import difflib
import os

import pytest

import difflib_mojo
from difflib_mojo import _native
from difflib_mojo._native import NativeUnavailable


def test_backend_info_shape():
    info = difflib_mojo.backend_info()
    assert info["abi_version_expected"] == _native.ABI_VERSION
    assert info["disabled_by_env"] == (
        os.environ.get("DIFFLIB_MOJO_DISABLE_NATIVE") == "1"
    )
    if info["disabled_by_env"]:
        assert info["native_available"] is False
    else:
        # The suite's native run requires a built kernel (built before pytest).
        assert info["native_available"] is True
        assert info["abi_version_native"] == _native.ABI_VERSION
        assert info["native_source"]


def test_env_forces_fallback(monkeypatch):
    monkeypatch.setenv("DIFFLIB_MOJO_DISABLE_NATIVE", "1")
    s = difflib_mojo.SequenceMatcher(None, "abcd", "bcde")
    assert s.ratio() == 0.75
    assert difflib_mojo.native_available() is False
    assert difflib_mojo.get_close_matches("appel", ["ape", "apple"]) == ["apple", "ape"]


def test_broken_override_falls_back_to_candidates(monkeypatch, tmp_path):
    # A corrupt/unloadable override must not crash matching: the resolver
    # skips it and continues down the candidate list.
    bogus = tmp_path / "not-a-real-lib.dylib"
    bogus.write_text("definitely not a mach-o")
    monkeypatch.setenv("DIFFLIB_MOJO_NATIVE_LIB", str(bogus))
    monkeypatch.delenv("DIFFLIB_MOJO_DISABLE_NATIVE", raising=False)
    _native._LIB, _native._LIB_SOURCE = None, None  # reset module cache
    try:
        assert difflib_mojo.SequenceMatcher(None, "abcd", "bcde").ratio() == 0.75
    finally:
        _native._LIB, _native._LIB_SOURCE = None, None


def test_no_candidates_raises_native_unavailable(monkeypatch):
    monkeypatch.delenv("DIFFLIB_MOJO_DISABLE_NATIVE", raising=False)
    monkeypatch.setattr(_native, "_candidate_paths", lambda: [])
    _native._LIB, _native._LIB_SOURCE = None, None
    try:
        with pytest.raises(NativeUnavailable):
            _native._load()
        # ... and the public API then falls back transparently.
        assert difflib_mojo.SequenceMatcher(None, "abcd", "bcde").ratio() == 0.75
        assert difflib_mojo.get_close_matches("appel", ["ape", "apple"]) == [
            "apple",
            "ape",
        ]
    finally:
        _native._LIB, _native._LIB_SOURCE = None, None


def test_abi_mismatch_rejected(monkeypatch):
    class FakeLib:
        def difflibmojo_abi_version(self):
            return _native.ABI_VERSION + 1

    monkeypatch.delenv("DIFFLIB_MOJO_DISABLE_NATIVE", raising=False)
    monkeypatch.setattr(_native.ctypes, "CDLL", lambda path: FakeLib())
    monkeypatch.setattr(
        _native, "_candidate_paths", lambda: [("fake", "/fake/libdifflibmojo.dylib")]
    )
    monkeypatch.setattr(_native.os.path, "exists", lambda p: True)
    monkeypatch.setattr(_native, "_bind_abi", lambda lib: None)
    _native._LIB, _native._LIB_SOURCE = None, None
    try:
        with pytest.raises(NativeUnavailable, match="ABI"):
            _native._load()
    finally:
        _native._LIB, _native._LIB_SOURCE = None, None


def test_non_str_uses_engine_on_both_backends():
    # Even with the native kernel loaded, non-str sequences route to the
    # vendored engine and agree with the reference.
    a = [1, 2, 3, 4, 5]
    b = [1, 2, 9, 4, 5, 6]
    got = difflib_mojo.SequenceMatcher(None, a, b)
    ref = difflib.SequenceMatcher(None, a, b)
    assert got.ratio() == ref.ratio()
    assert [tuple(t) for t in got.get_matching_blocks()] == [
        tuple(t) for t in ref.get_matching_blocks()
    ]
    assert got.get_opcodes() == ref.get_opcodes()
