#!/usr/bin/env bash
# Compile the difflib Mojo kernel into a shared library (macOS .dylib /
# Linux .so). Run inside the repo pixi environment
# (`pixi run bash kernels/difflib/build.sh`).
set -euo pipefail
...
mojo build --emit shared-lib -o "build/${lib}" src/difflibmojo.mojo
echo "built kernels/difflib/build/${lib}"
