#!/usr/bin/env bash
# Compile the ruptures Mojo kernel into a shared library (macOS .dylib /
# Linux .so). Run inside the repo pixi environment:
#   ~/.pixi/bin/pixi run bash kernels/ruptures/build.sh
set -euo pipefail
...
mojo build --emit shared-lib -o "build/${lib}" src/rupturesmojo.mojo
echo "built kernels/ruptures/build/${lib}"
