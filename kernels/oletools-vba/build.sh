#!/usr/bin/env bash
# Compile the MS-OVBA (VBA decompression) Mojo kernel into a shared library
# (macOS .dylib / Linux .so). Run inside the repo pixi environment
# (`pixi run bash kernels/oletools-vba/build.sh`).
set -euo pipefail
...
mojo build --emit shared-lib -o "build/${lib}" src/vbamojo.mojo
echo "built kernels/oletools-vba/build/${lib}"
