#!/usr/bin/env bash
# Compile the elephant-surrogates Mojo kernel into a shared library
...
case "$(uname -s)" in
  Darwin)
    lib="libelephantsurrogatesmojo.dylib"
    ...
    ;;
  Linux)  lib="libelephantsurrogatesmojo.so" ;;
  *)
...
mojo build --emit shared-lib -o "build/${lib}" src/elephantsurrogatesmojo.mojo
echo "built kernels/elephant-surrogates/build/${lib}"
