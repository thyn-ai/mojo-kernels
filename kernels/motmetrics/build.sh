#!/usr/bin/env bash
# Compile the motmetrics Mojo kernel into a shared library
# (macOS .dylib / Linux .so). Run inside the repo pixi environment
# (`pixi run bash kernels/motmetrics/build.sh` from the repository root).
set -euo pipefail
...
mojo build --emit shared-lib -o "build/${lib}" src/motmojo.mojo
echo "built kernels/motmetrics/build/${lib}"
