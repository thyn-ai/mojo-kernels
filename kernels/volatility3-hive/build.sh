#!/usr/bin/env bash
# Compile the volatility3-hive Mojo kernel into a shared library
# (macOS .dylib / Linux .so). Run inside the repo pixi environment, e.g.
#   pixi run bash kernels/volatility3-hive/build.sh
set -euo pipefail
...
case "$(uname -s)" in
  Darwin)
    lib="libvolatility3hivemojo.dylib"
    ...
    ;;
  Linux)  lib="libvolatility3hivemojo.so" ;;
  *)
...
mojo build --emit shared-lib -o "build/${lib}" src/volatility3hivemojo.mojo
echo "built kernels/volatility3-hive/build/${lib}"
