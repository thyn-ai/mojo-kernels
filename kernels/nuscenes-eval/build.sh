#!/usr/bin/env bash
# Compile the nuScenes-eval Mojo kernel into a shared library (macOS .dylib /
...
case "$(uname -s)" in
  Darwin)
    ...
    ;;
  Linux)  lib="libnuscenesevalmojo.so" ;;
  *)
...
mojo build --emit shared-lib -o "build/${lib}" src/nuscenesevalmojo.mojo
echo "built kernels/nuscenes-eval/build/${lib}"
