#!/usr/bin/env bash
# Compile the pypdf-filters Mojo kernel into a shared library
# (macOS .dylib / Linux .so). Run inside the repo pixi environment:
#   pixi run bash kernels/pypdf-filters/build.sh
set -euo pipefail
...
mojo build --emit shared-lib -o "build/${lib}" src/pdfmojo.mojo
echo "built kernels/pypdf-filters/build/${lib}"
