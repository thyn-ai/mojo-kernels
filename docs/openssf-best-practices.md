...
4. Four SUGGESTED criteria are Unmet (`version_tags`, `warnings_strict`,
   `dynamic_analysis`, `dynamic_analysis_unsafe`). Each row names the change
   that would meet it.
...
| `version_unique` (MUST) | Met | Each package carries a Semantic Versioning identifier (`version = "0.1.0"` in `python/*/pyproject.toml`, `"version": "0.1.0"` in `typescript/fuse-mojo/packages/*/package.json`); `GOVERNANCE.md` requires releases to be cut from a tagged commit and release-drafter names releases `v$RESOLVED_VERSION`. No release has shipped yet; the first will be `v0.1.0`. `R/blob/main/GOVERNANCE.md#releases` |
...
| `version_tags` (SUGGESTED) | **Not met** | The repository has no git tags yet (release-drafter holds an unpublished draft). Would meet it: publish the first release from the draft, which creates the `v0.1.0` tag (`tag-template: v$RESOLVED_VERSION` in `.github/release-drafter.yml`); make sure the resolved version matches the packages' `0.1.0`. |
| `release_notes` (MUST, URL required) | **Not met** (no release yet) | `CHANGELOG.md` follows Keep a Changelog with a human-written `[Unreleased]` summary of each package, and release-drafter maintains a categorized draft release on every merge - but no release has been published, so there are no release notes *in a release*. Would meet it: at the first release, move `[Unreleased]` to `## [0.1.0] - <date>` and publish the GitHub Release with that summary as its body (the drafter's list of pull-request titles is not a substitute for the summary). `R/blob/main/CHANGELOG.md`, `R/releases` |
...
| Criterion | Status | Justification and evidence |
|---|---|---|
| `build` (MUST) | Met | One reproducible build system: `pixi.toml` / `pixi.lock` pin the Mojo compiler, Python and every test oracle; `pixi run build-kernel-{bm25,gaussgrid,fuse}` compile the kernels, `pixi run wheel-{bm25,cclib}` build and repair the wheels, `pixi run pack-fuse` builds the npm platform package. CI runs exactly these tasks. `R/blob/main/pixi.toml`, `R/blob/main/CONTRIBUTING.md#running-the-same-commands-ci-runs` |
...
| `test` (MUST) | Met | Automated suites in the repository, under the project's Apache-2.0 license: pytest differential suites for bm25-mojo and cclib-mojo (`tests/`) and a `node --test` differential + unit suite for fuse-mojo (`typescript/fuse-mojo/tests/`), each run twice (native kernel, then forced fallback) against the real reference package. How to run them is documented (`pixi run test`, `pixi run test-cclib`, `pixi run test-fuse`) and is what CI runs. `R/blob/main/CONTRIBUTING.md#running-the-same-commands-ci-runs`, `R/tree/main/tests` |
...
| `test_continuous_integration` (SUGGESTED) | Met | Three workflows (`ci.yml`, `ci-cclib.yml`, `ci-fuse.yml`) run on every pull request and every push to `main`, on `ubuntu-latest` and `macos-latest`, and are required status checks in the branch ruleset. `R/actions` |
...
| `tests_are_added` (MUST) | Met | Each of the three kernels landed together with its differential suite and loader tests (`tests/test_differential.py` + `tests/test_loader.py` for bm25, `tests/test_gaussgrid_differential.py` + `tests/test_gaussgrid_loader.py` for cclib, `typescript/fuse-mojo/tests/*.test.cjs` for fuse), and CI runs them on both backends. `R/tree/main/tests` |
...
| `delivery_unsigned` (MUST) | Met | No hash is ever fetched over plain HTTP: every pinned artifact (`pixi.lock`, `typescript/fuse-mojo/package-lock.json`, the SHA-pinned GitHub Actions) is retrieved over HTTPS and verified against the hash recorded in the repository; the end-user smoke installs with `npm ci` so the tarballs and koffi are hash-verified too. Release signing (sigstore) is a planned improvement tracked by Scorecard's `Signed-Releases` check and is not required here. `R/blob/main/typescript/fuse-mojo/scripts/smoke.sh` |
| `vulnerabilities_fixed_60_days` (MUST) | Met | No vulnerability of any severity is publicly known against this project: 0 security advisories, 0 open Dependabot alerts, 0 open code-scanning alerts (checked 2026-09-19). Dependabot security updates are enabled with weekly version updates for GitHub Actions, npm and both pip manifests. `R/security`, `R/blob/main/.github/dependabot.yml` |
...
| `no_leaked_credentials` (MUST) | Met | The repository contains no credentials (CI runs with no secrets and a read-only token). GitHub secret scanning, push protection, validity checks and non-provider patterns are enabled on the repository; `CONTRIBUTING.md` forbids hard-coded tokens or endpoints. `R/blob/main/CONTRIBUTING.md#what-a-kernel-change-needs` |

## Analysis

| Criterion | Status | Justification and evidence |
|---|---|---|
| `static_analysis` (MUST) | Met | CodeQL (advanced setup) analyses the four languages CodeQL supports here - `python`, `javascript-typescript`, `c-cpp`, `actions` - on every pull request to `main`, every push to `main`, and weekly. The branch ruleset makes the four `CodeQL (<language>)` jobs required checks and blocks merging on any error/warning alert or any medium-or-higher security alert. Mojo is not a CodeQL language; the kernels are covered by the differential suites, which is stated in the workflow. `R/blob/main/.github/workflows/codeql.yml`, `R/security/code-scanning` |
...
| `static_analysis_fixed` (MUST) | Met | 0 open code-scanning alerts; the ruleset's code-scanning rule prevents merging while a medium-or-higher alert exists on the pull request. `R/security/code-scanning` |
...
| `dynamic_analysis` (SUGGESTED) | **Not met** | The differential suites are dynamic tests with seeded, generated corpora, but no fuzzing or input-varying analysis tool runs. Would meet it: an atheris harness for the Python wrappers and a fast-check property suite for fuse-mojo, run in CI (this also lifts Scorecard's `Fuzzing` check). |
...
| `dynamic_analysis_fixed` (MUST) | N/A | No dynamic analysis tool is run yet, so no vulnerabilities have been found this way (the criterion allows N/A in that case). Becomes Met once `dynamic_analysis` is addressed, provided findings are fixed in a timely way. |
