...
| `ill-conditioned-norm` | `fuzz_bm25.py` | [#58](https://github.com/thyn-ai/mojo-kernels/issues/58) -- raw parameters that cancel the length normaliser `1 - b + b * dl / avgdl` (or the outer denominator) by 1e13 or more leave rounding residue in place of the score: for any `b` of magnitude above 2^53 the normaliser is 0 or one ulp at every document of average length while its exact value is 1. The kernel's baked BM25Plus floor decomposition and the fallback's single evaluation round that residue differently (1e7 tolerances apart at 1e157 in the unit that found it). The divergence must be confined to the cancelled documents that contain the term (documents without it are the `degenerate-nan` 0/0 shape, which the same query may show at the same time); in-domain parameters (`b` in [0, 1], `k1`, `epsilon`/`delta` >= 0) have no cancelling summand and can never reach it |
